...
from dynamics import SingleIntegrator, Unicycle, DoubleIntegrator, Dyn
import abc
...
class Sphere(Obstacle):
    ...
    def h_func(self, a1):
        ...
        # print('-----')
        # print(self.state)
        return ((a1.state-self.state).T.dot(self.M).dot((a1.state-self.state))[0][0] - (self.sign*a1.radius+self.radius)**2) * self.sign
        
    def h_func_grad(self, a1):
        return (2*self.M.dot((a1.state - self.state))) * self.sign
...
class Ellipsoid(Obstacle):
    ...
    def h_func(self, a1):
        return ((a1.state-self.state).transpose().dot(self.get_M(a1)).dot((a1.state-self.state))[0][0] - 1) * self.sign
